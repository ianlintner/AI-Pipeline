...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bug_report_service.log'),
...
class BugReportTriageService:
    ...
    def submit_bug_report(self, bug_report: BugReport) -> Optional[str]:
        ...
        try:
            ...
            if request_id:
                logger.info(f"Bug report submitted successfully with request ID: {request_id}")
            return request_id
        except Exception as e:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            health_status = {
                ...
                "active_requests": len(self.coordinator.active_requests) if self.coordinator else 0
            }
            ...
            for name, agent in self.agents.items():
                try:
                    ...
                    component_status[f"{name}_agent"] = {
                        "status": "healthy" if hasattr(agent, 'agent_name') else "unhealthy",
