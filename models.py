...
class BugReport(BaseModel):
    ...
    reporter: str = Field(..., description="Name or email of the person reporting the bug")
    environment: Optional[str] = Field(None, description="Environment where bug occurred")
    ...
    attachments: Optional[List[str]] = Field(default_factory=list, description="List of attachment URLs")
...
class TriageResult(BaseModel):
    ...
    priority: Priority
    severity: Severity
    category: str = Field(..., description="Category of the bug (e.g., 'frontend', 'backend', 'database')")
    labels: List[str] = Field(default_factory=list, description="Labels to be applied to GitHub issue")
    assignee_suggestion: Optional[str] = Field(None, description="Suggested assignee")
    duplicate_of: Optional[str] = Field(None, description="ID of duplicate issue if applicable")
